...
func main() {
	var proxy = false
	flag.BoolVar(&proxy, "proxy-request", proxy, "forward requests to database extended apiserver")
	flag.Parse()
	fmt.Println(proxy)
	...
	if proxy {
		easStore, err := certstore.NewCertStore(fs, "/tmp/k8s-extended-apiserver-DIY")
		if err != nil {
			log.Fatalln(err)
		}
	...
	if proxy {
		fmt.Println("Entered in forwarding")
